...
APPBASE="build/macos-aarch64/Veskar.app"

build() {
    echo Launcher sha256sum
    shasum -a 256 build/libs/Veskar.jar
    ...
    cp native/build-aarch64/src/Veskar $APPBASE/Contents/MacOS/
    cp build/libs/Veskar.jar $APPBASE/Contents/Resources/
    ...
    echo Setting world execute permissions on Veskar
    pushd $APPBASE
    chmod g+x,o+x Contents/MacOS/Veskar
    popd

    otool -l $APPBASE/Contents/MacOS/Veskar
}
...
dmg() {
    ...
    mv Veskar\ *.dmg Veskar-aarch64.dmg

    # dump for CI
    hdiutil imageinfo Veskar-aarch64.dmg

    if ! hdiutil imageinfo Veskar-aarch64.dmg | grep -q "Format: ULFO" ; then
        echo Format of dmg is not ULFO
        exit 1
    fi

    if ! hdiutil imageinfo Veskar-aarch64.dmg | grep -q "Apple_HFS" ; then
        echo Filesystem of dmg is not Apple_HFS
        exit 1
    fi

    # Notarize app
    if xcrun notarytool submit Veskar-aarch64.dmg --wait --keychain-profile "AC_PASSWORD" ; then
        xcrun stapler staple Veskar-aarch64.dmg
    fi
}
